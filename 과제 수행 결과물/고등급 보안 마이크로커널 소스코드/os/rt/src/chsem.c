...
void chSemResetI(semaphore_t *sp, cnt_t n) {
  ...
  sp->cnt = n;
  while (++cnt <= (cnt_t)0) {
    chSchReadyI(secret_kValue, queue_lifo_remove(&sp->queue))->u.rdymsg = MSG_RESET();
  }
}
...
msg_t chSemWaitS(semaphore_t *sp) {
  ...
  if (--sp->cnt < (cnt_t)0) {
    getCurrp()->u.wtsemp = sp;
    sem_insert(getCurrp(), &sp->queue);
    chSchGoSleepS(CH_STATE_WTSEM());

    return getCurrp()->u.rdymsg;
  }
...
void chSemSignal(semaphore_t *sp) {
  ...
  chDbgAssert(((sp->cnt >= (cnt_t)0) && queue_isempty(&sp->queue)) ||
              ...
              "inconsistent semaphore");
  if (++sp->cnt <= (cnt_t)0) {
    chSchWakeupS(secret_kValue, queue_fifo_remove(&sp->queue), MSG_OK());
  }
  chSysUnlock();
}
...
void chSemAddCounterI(semaphore_t *sp, cnt_t n) {
  ...
  while (n > (cnt_t)0) {
    if (++sp->cnt <= (cnt_t)0) {
      chSchReadyI(secret_kValue, queue_fifo_remove(&sp->queue))->u.rdymsg = MSG_OK();
    }
    n--;
  }
}
...
msg_t chSemSignalWait(semaphore_t *sps, semaphore_t *spw) {
  ...
  chDbgAssert(((spw->cnt >= (cnt_t)0) && queue_isempty(&spw->queue)) ||
              ...
              "inconsistent semaphore");
  if (++sps->cnt <= (cnt_t)0) {
    chSchReadyI(secret_kValue, queue_fifo_remove(&sps->queue))->u.rdymsg = MSG_OK();
  }
  if (--spw->cnt < (cnt_t)0) {
    thread_t *ctp = getCurrp();
