...
void THD_WORKING_AREA_SIZE(int n);
...
extern "C" {
  ...
  void chThdSleep(sysinterval_t time); // SecretValue_t kValue 우선 보류 (TODO)
  void chThdSleepUntil(systime_t time); // SecretValue_t kValue 우선 보류 (TODO)
  systime_t chThdSleepUntilWindowed(systime_t prev, systime_t next); // SecretValue_t kValue 우선 보류 (TODO)
  void chThdYield(void);

  // Module inline functions (Static inline -> Extern)
...
static inline bool chThdTerminatedX(thread_t *tp) {
  ...
  return (bool)(thread_get_state(tp) == CH_STATE_FINAL());
}
...
static inline bool chThdShouldTerminateX(void) {
  ...
  return (bool)((thread_get_flags(chThdGetSelfX()) & CH_FLAG_TERMINATE()) != (tmode_t)0);
}
 ...
 * @iclass
 */
static inline thread_t *chThdStartI(SecretValue_t kValue, thread_t *tp) {

  chDbgAssert(thread_get_state(tp) == CH_STATE_WTSTART(), "wrong state");

  return chSchReadyI(kValue, tp);
}
 ...
 * @sclass
 */
static inline void chThdSleepS(SecretValue_t kValue, sysinterval_t ticks) {
 ...
 * @iclass
 */
static inline void chThdDoDequeueNextI(SecretValue_t kValue, threads_queue_t *tqp, msg_t msg) {
  thread_t *tp;
  ...
  chDbgAssert(thread_get_state(tp) == CH_STATE_QUEUED(), "invalid state");
