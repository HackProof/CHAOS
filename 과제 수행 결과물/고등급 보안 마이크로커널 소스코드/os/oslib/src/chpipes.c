...
void chPipeReset(pipe_t *pp) {
  ...
  chSysLock();
  chThdResumeI(secret_kValue, &pp->wtr, MSG_RESET());
  chThdResumeI(secret_kValue, &pp->rtr, MSG_RESET());
  chSchRescheduleS();
  chSysUnlock();
...
size_t chPipeWriteTimeout(pipe_t *pp, const uint8_t *bp,
  ...
  while (n > 0U) {
    ...
    if (done == (size_t)0) {
      ...
      chThdResume(secret_kValue, &pp->rtr, MSG_OK());
    }
  }
...
size_t chPipeReadTimeout(pipe_t *pp, uint8_t *bp,
  ...
  while (n > 0U) {
    ...
    if (done == (size_t)0) {
      ...
      chThdResume(secret_kValue, &pp->wtr, MSG_OK());
    }
  }
