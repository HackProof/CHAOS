...
static THD_FUNCTION(bmk_thread3, p) {
  ...
  chThdExit(secret_kValue, (msg_t)p);
}
...
static void rt_test_010_001_execute(void) {
  ...
  test_set_step(1);
  {
    threads[0] = chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, chThdGetPriorityX()-1, bmk_thread1, NULL);
  }
...
static void rt_test_010_003_execute(void) {
  ...
  test_set_step(2);
  {
    threads[1] = chThdCreateStatic(secret_kValue, secret_kValue, wa[1], WA_SIZE, chThdGetPriorityX()-2, bmk_thread3, NULL);
    threads[2] = chThdCreateStatic(secret_kValue, secret_kValue, wa[2], WA_SIZE, chThdGetPriorityX()-3, bmk_thread3, NULL);
    threads[3] = chThdCreateStatic(secret_kValue, secret_kValue, wa[3], WA_SIZE, chThdGetPriorityX()-4, bmk_thread3, NULL);
    threads[4] = chThdCreateStatic(secret_kValue, secret_kValue, wa[4], WA_SIZE, chThdGetPriorityX()-5, bmk_thread3, NULL);
  }
...
static void rt_test_010_004_execute(void) {
  ...
  test_set_step(1);
  {
    tp = threads[0] = chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, chThdGetPriorityX()+1,
                                        bmk_thread4, NULL);
  }
  ...
  test_set_step(2);
  {
    ...
    do {
      chSysLock();
      chSchWakeupS(secret_kValue, tp, MSG_OK());
      chSchWakeupS(secret_kValue, tp, MSG_OK());
      chSchWakeupS(secret_kValue, tp, MSG_OK());
      chSchWakeupS(secret_kValue, tp, MSG_OK());
      chSysUnlock();
  ...
  {
    chSysLock();
    chSchWakeupS(secret_kValue, tp, MSG_TIMEOUT());
    chSysUnlock();
    test_wait_threads();
  }
...
static void rt_test_010_005_execute(void) {
  // printf("test_010_005_execute start\n");
  uint32_t n;
  ...
  {
    // printf("test %d pass.\n", n);
    // test_println("test_010_005_execute step 1 start"); // park, seonghoon
    ...
    do {
      // printf("test %d start.\n", n);
      chThdWait(secret_kValue, chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, prio, bmk_thread3, NULL));
      n++;

      // test_print("test number: "); // park, seonghoon
      // test_printn(n); // park, seonghoon
      // test_println(" passed!"); // park, seonghoon
#if defined(SIMULATOR)
      _sim_check_for_interrupts();
#endif
    } while (chVTIsSystemTimeWithinX(start, end));
...
static void rt_test_010_006_execute(void) {
  ...
  test_set_step(1);
  {
    // test_println("test_010_006_execute step 1 start"); // park, seonghoon
    ...
    do {
      ...
      chThdRelease(chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, prio, bmk_thread3, NULL));
#else
      chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, prio, bmk_thread3, NULL);
#endif
      n++;
#if defined(SIMULATOR)
      _sim_check_for_interrupts();
...
static void rt_test_010_007_execute(void) {
  ...
  test_set_step(1);
  {
    threads[0] = chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, chThdGetPriorityX()+5, bmk_thread7, NULL);
    threads[1] = chThdCreateStatic(secret_kValue, secret_kValue, wa[1], WA_SIZE, chThdGetPriorityX()+4, bmk_thread7, NULL);
    threads[2] = chThdCreateStatic(secret_kValue, secret_kValue, wa[2], WA_SIZE, chThdGetPriorityX()+3, bmk_thread7, NULL);
    threads[3] = chThdCreateStatic(secret_kValue, secret_kValue, wa[3], WA_SIZE, chThdGetPriorityX()+2, bmk_thread7, NULL);
    threads[4] = chThdCreateStatic(secret_kValue, secret_kValue, wa[4], WA_SIZE, chThdGetPriorityX()+1, bmk_thread7, NULL);
  }
...
static void rt_test_010_008_execute(void) {
  ...
  {
    n = 0;
    test_wait_tick();threads[0] = chThdCreateStatic(secret_kValue, secret_kValue, wa[0], WA_SIZE, chThdGetPriorityX()-1, bmk_thread8, (void *)&n);

    threads[1] = chThdCreateStatic(secret_kValue, secret_kValue, wa[1], WA_SIZE, chThdGetPriorityX()-1, bmk_thread8, (void *)&n);
    threads[2] = chThdCreateStatic(secret_kValue, secret_kValue, wa[2], WA_SIZE, chThdGetPriorityX()-1, bmk_thread8, (void *)&n);
    threads[3] = chThdCreateStatic(secret_kValue, secret_kValue, wa[3], WA_SIZE, chThdGetPriorityX()-1, bmk_thread8, (void *)&n);
    threads[4] = chThdCreateStatic(secret_kValue, secret_kValue, wa[4], WA_SIZE, chThdGetPriorityX()-1, bmk_thread8, (void *)&n);
  }
