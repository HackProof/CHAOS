...
static void rt_test_009_001_execute(void) {
  ...
  test_set_step(3);
  {
    threads[0] = chThdCreateFromHeap(secret_kValue, &heap1,
  ...
  test_set_step(4);
  {
    threads[1] = chThdCreateFromHeap(secret_kValue, &heap1,
                                     THD_WORKING_AREA_SIZE(THREADS_STACK_SIZE),
                                     "dyn2",
  ...
  test_set_step(5);
  {
    threads[2] = chThdCreateFromHeap(secret_kValue, &heap1,
                                     (((size_t)-1) >> 1U) + 1U,
                                     "dyn3",
...
static void rt_test_009_002_execute(void) {
  ...
  test_set_step(3);
  {
    threads[0] = chThdCreateFromMemoryPool(secret_kValue, &mp1, "dyn1", prio-1, dyn_thread1, "A");
    threads[1] = chThdCreateFromMemoryPool(secret_kValue, &mp1, "dyn2", prio-2, dyn_thread1, "B");
    threads[2] = chThdCreateFromMemoryPool(secret_kValue, &mp1, "dyn3", prio-3, dyn_thread1, "C");
    threads[3] = chThdCreateFromMemoryPool(secret_kValue, &mp1, "dyn4", prio-4, dyn_thread1, "D");
    threads[4] = chThdCreateFromMemoryPool(secret_kValue, &mp1, "dyn5", prio-5, dyn_thread1, "E");
    test_println("3단계 성공!");
  }
