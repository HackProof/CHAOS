...
static void termination_handler(eventid_t id) {
  ...
  if (shelltp1 && chThdTerminatedX(shelltp1)) {
    chThdWait(secret_kValue, shelltp1);
    shelltp1 = NULL;
  ...
  if (shelltp2 && chThdTerminatedX(shelltp2)) {
    chThdWait(secret_kValue, shelltp2);
    shelltp2 = NULL;
    chThdSleepMilliseconds(10);
...
static void sd1_handler(eventid_t id) {
  ...
  if ((flags & CHN_CONNECTED) && (shelltp1 == NULL)) {
    ...
    shelltp1 = chThdCreateFromHeap(secret_kValue, NULL, SHELL_WA_SIZE,
...
static void sd2_handler(eventid_t id) {
  ...
  if ((flags & CHN_CONNECTED) && (shelltp2 == NULL)) {
    ...
    shelltp2 = chThdCreateFromHeap(secret_kValue, NULL, SHELL_WA_SIZE,
...
int main(void) {
   ...
   */
  cdtp = chThdCreateFromHeap(secret_kValue, NULL, CONSOLE_WA_SIZE, "console",
